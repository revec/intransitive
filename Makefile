...
testbeds:
	find tests -name "testbed.ll" | sort | xargs -n 1 -P `nproc` -I % llc-6.0 % -O0 -mcpu=skylake
	find tests -name "testbed.s" | sort | xargs -n 1 -P `nproc` -I % sh -c "as % --64 -o \"\`dirname %\`/testbed.o\";"
	find tests -name "testbed.o" | sort | xargs -n 1 -P `nproc` -I % sh -c "gcc -m64 % -o \`dirname %\`/testbed;"
